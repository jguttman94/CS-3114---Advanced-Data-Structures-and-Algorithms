...
public class Hash {
     ...
     * appropriate size
     * 
     * @param size size of the hash tables at initialization, passed from parser
    ...
    public boolean insert(String name) {
        ...
        if (check(name) == -1) {
            ...
            if (temp != -1) {
                hashTable[temp] = memMan.insert(name.getBytes(), name.length());
                setHand(hashTable[temp]);
                hashSize++;
                return true;
            }
            // probing failed
    ...
    public boolean resize() {
        ...
        if (getHashSize() >= (hashTable.length / 2)) {
            ...
            for (int i = 0; i < cleanArray.length; i++) {
                if (hashTable[i] != null && hashTable[i].getStartPos() == -1) {
                    cleanArray[i] = null;
                }
                else {
     ...
     * @return int the index found by probing that is empty, -1 if probe failed
     */
    public int probe(String name) {
        ...
        for (int counter = 0; counter < hashTable.length; counter++) {
            if (hashTable[tempHome] != null
                    && hashTable[tempHome].getStartPos() != -1) {
                tempHome = ((home + (counter * counter)) % hashTable.length);
            }
            else {
                return tempHome;
    ...
    // ----------------------------------------------------------------------
    /**
     * Prints the current Artists found in order and the total number on record.
     * 
     ...
     */
    public void setHand(Handle hand) {
        this.hand = hand;
    }
}
